...
### Phase 7, slice 18: texture generators — 10.6k sprites, byte-for-byte, and the trim materials

The texture half of client datagen runs: MaterialPartTextureGenerator (the recolored tool part
sprites — every part crossed with every material's grey-to-color mapping), MaterialRenderInfoProvider
(the {@code tinkering/materials} JSONs including their generator blocks), the part-texture manifest
(GeneratorPartTextureJsonGenerator), ArmorModelProvider, the trim color palettes
(TinkerTrimMaterialPaletteGenerator, including the {@code trimmed} namespace map), the palette
debug sheets, and the fluid texture pair: AbstractFluidTextureProvider (the
{@code mantle/fluid_texture} JSONs) and FluidTextureCameraProvider (16x16 first-frame camera
overlays, via a new mantle {@code DeanimateTextureGenerator}).

**Verification, clean-room run (generated tree removed from the existing roots so the
incremental generator regenerates everything): 11,021 asset files — 10,181 tool part sprites,
351 armor textures, 6 storage fallbacks, 61 fluid textures, 16 trim palettes, all JSON layers —
0 missing, 0 extra, 0 binary diffs, and after one ordering fix 0 JSON diffs. Every PNG matches
the Forge-generated bytes** (same stb writer, same pixels). The one divergence found and fixed:
the BODY-exclusion rewrite of the armor loop in TinkerPartSpriteProvider had inverted the slot
order (boots-first), scrambling the manifest's array order against the committed file.

**TrimMaterialProvider closes the last server-data gap**: it is a datapack-registry provider
({@code BootstapContext} became {@code BootstrapContext}), now wired through
{@code buildRegistry}; the 16 {@code trim_material} JSONs regenerate identically — nothing left
missing from the data tree but the parked structure NBTs.

Porting notes:
- The sprite generator is **incremental by design**: it skips any sprite that already exists in
  the existing-file helper, which is what lets artists override generated textures and keeps
  the steady-state {@code runDatagen} fast (the committed tree stays authoritative). The
  verification run drops {@code src/generated/resources} from the roots to force a full rebuild.
- Fluid textures re-key from Forge's fluid-type registry to the still fluid's id — matching
  {@link slimeknights.mantle.fluid.texture.FluidTextureManager}'s runtime keying and the shipped
  file names. {@code FluidTexture.Builder.wrapId} works again on that id. Milk is skipped with a
  PORT note: on Forge it used the shared forge fluid type this provider never covered, and its
  {@code milk.json} ships as a static asset.
- The two {@code Client*FluidType} classes stay dropped (Forge {@code IClientFluidTypeExtensions};
  the Fabric render handler covers their sprite/tint duties).
...
### Phase 7, slice 20: the documentation — and the port is complete

The closing requirement: full documentation with every recipe. `docs/` now carries it:

- **`docs/reference/`** — generated by `scripts/generate_recipe_docs.py` from
  `src/generated/resources`, the same files the game loads. All 3,036 recipes are covered
  with a hard completeness check in the generator (it fails if any recipe is unaccounted
  for): melting with temperatures/byproducts/compat conditions, all alloys, casting, entity
  melting, every modifier recipe with inputs and slot costs, every material with stats and
  traits, tool definitions, and a grouped sweep of everything else.
- **`docs/PORT.md`** — the distilled port: what runs, the Forge-to-Fabric concept map,
  the verification harnesses, known deviations, repository layout.
- **`docs/PACK-INTEGRATION.md`** — modpack guidance: install, tagPreferences/Unify
  alignment, the tag-driven compat metals, and the `/mantle remove_recipes` workflow for
  replacing vanilla tools.

With this, phase 7 closes and so does the port plan: every phase checked. What remains
in `unported.gradle` is a dozen deliberately dropped classes, each with its reason inline
...
- [x] **7 — Datagen & documentation.** Datagen complete (slices 15-19): everything under src/generated/resources regenerates on Fabric, byte-identical or better. Documentation complete (slice 20): docs/ with the generated all-recipe reference, the port map and pack integration.
