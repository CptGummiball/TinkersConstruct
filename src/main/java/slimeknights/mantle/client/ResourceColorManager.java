...
public class ResourceColorManager implements ISafeManagerReloadListener {
  ...
  public static void init() {
    // Reload on tag load, which fires for resource and datapack reloads alike.
    CommonLifecycleEvents.TAGS_LOADED.register((registries, client) -> {
      if (client) {
        INSTANCE.onResourceManagerReload(net.minecraft.client.Minecraft.getInstance().getResourceManager());
      }
    });
  }
    ...
    for (Entry<String,JsonElement> entry : json.entrySet()) {
      ...
      if (element.isJsonObject()) {
        ...
        if (!colors.containsKey(fullPath)) {
          ...
          // 1.21 parseColor returns a DataResult
