...
public class GeneratePackHelper {
  ...
  public static final DynamicCommandExceptionType FAILED_SAVE = new DynamicCommandExceptionType(id -> net.minecraft.network.chat.Component.translatable("command.mantle.remove.failed_save", String.valueOf(id)));
  ...
  public static MutableComponent getPathComponent(MutableComponent text, String path) {
    return text.withStyle(style -> style.withUnderlined(true).withClickEvent(new ClickEvent(Action.COPY_TO_CLIPBOARD, path))); // PORT: 1.21 rejects OPEN_FILE in server-sent chat, so the click copies the path
  }
}
