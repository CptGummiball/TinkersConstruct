...
public abstract class InventoryBlock extends Block implements EntityBlock {
  ...
  protected boolean openGui(Player player, Level world, BlockPos pos) {
    if (!world.isClientSide()) {
      ...
      if (container != null && player instanceof ServerPlayer serverPlayer) {
        // Forge wrote the position into the opening packet; on Fabric the factory carries
        // it as typed data, read back by the menu's client constructor.
        serverPlayer.openMenu(new ExtendedScreenHandlerFactory<BlockPos>() {
          ...
          @Override
          public net.minecraft.world.inventory.AbstractContainerMenu createMenu(int syncId, net.minecraft.world.entity.player.Inventory inventory, Player player) {
            return container.createMenu(syncId, inventory, player);
          }
        });
  ...
  public void onRemove(BlockState state, Level worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
    if (state.getBlock() != newState.getBlock()) {
      ...
      if (te != null) {
        slimeknights.mantle.transfer.TransferUtil.getItemHandler(worldIn, pos, null).ifPresent(inventory -> dropInventoryItems(state, worldIn, pos, inventory));
