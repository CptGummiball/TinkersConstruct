...
public class EmptyPotionTransfer extends EmptyFluidContainerTransfer {
  ...
  protected FluidStack getFluid(ItemStack stack) {
    ...
    return TagPreference.getPreference(MantleTags.Fluids.POTION)
      .map(value -> new FluidStack(value, fluid.getAmount(), potionTag(stack)))
      .orElse(FluidStack.EMPTY);
  }

  /** Rebuilds the legacy {@code {Potion: id}} tag the potion fluid reads from POTION_CONTENTS. */
  private static net.minecraft.nbt.CompoundTag potionTag(ItemStack stack) {
    ...
    stack.getOrDefault(DataComponents.POTION_CONTENTS, PotionContents.EMPTY).potion()
      ...
      .ifPresent(key -> tag.putString("Potion", key.location().toString()));
    return tag;
  }
