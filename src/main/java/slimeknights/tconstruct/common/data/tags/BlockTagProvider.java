...
import slimeknights.mantle.data.BlockTagsProvider;
...
import static slimeknights.mantle.datagen.Tags.Blocks.NEEDS_GOLD_TOOL;
import static slimeknights.mantle.datagen.Tags.Blocks.NEEDS_NETHERITE_TOOL;
...
public class BlockTagProvider extends BlockTagsProvider {
  ...
  private void addTools() {
    ...
    this.tag(TinkerTags.Blocks.WORKBENCHES)
        ...
        .addOptionalTag(ResourceLocation.parse("c:workbench")); // some mods use a non-standard name here, so support it I guess
  ...
  private void addHarvest() {
    ...
    // PORT: carried from the migrated data — slime dirt behaves as vanilla dirt, and the
    // vanilla storage tags forge used to ship come from us now
    this.tag(BlockTags.DIRT).add(TinkerWorld.slimeDirt.get(slimeknights.tconstruct.world.block.DirtType.EARTH), TinkerWorld.slimeDirt.get(slimeknights.tconstruct.world.block.DirtType.SKY), TinkerWorld.slimeDirt.get(slimeknights.tconstruct.world.block.DirtType.ICHOR), TinkerWorld.slimeDirt.get(slimeknights.tconstruct.world.block.DirtType.ENDER));
    this.tag(TagKey.create(Registries.BLOCK, Mantle.commonResource("storage_blocks/amethyst"))).add(Blocks.AMETHYST_BLOCK);
    this.tag(TagKey.create(Registries.BLOCK, Mantle.commonResource("storage_blocks/quartz"))).add(Blocks.QUARTZ_BLOCK);
    this.tag(TagKey.create(Registries.BLOCK, Mantle.commonResource("storage_blocks"))).addTag(TagKey.create(Registries.BLOCK, Mantle.commonResource("storage_blocks/amethyst"))).addTag(TagKey.create(Registries.BLOCK, Mantle.commonResource("storage_blocks/quartz")));
    // harvest tiers on shovel blocks
    TinkerWorld.slimeDirt.forEach((type, block) -> this.tag(Objects.requireNonNull(tierTag(type.getHarvestTier()))).add(block));
    for (DirtType dirt : DirtType.values()) {
      for (FoliageType grass : FoliageType.values()) {
        ...
        this.tag(Objects.requireNonNull(tierTag(tier))).add(TinkerWorld.slimeGrass.get(dirt).get(grass));
      }
    }
    ...
    // added by sword effective tag; PORT: removal is a provider method now, vanilla appenders cannot be extended
    removeFromTag(TinkerTags.Blocks.MINABLE_WITH_SCYTHE, Blocks.PUMPKIN, Blocks.CARVED_PUMPKIN, Blocks.MELON);
  ...
  /**
   * PORT: Forge patched {@code Tiers.getTag()} (the needs-x-tool block tag) and
   * {@code getLevel()} onto tiers; 1.21 vanilla tiers only carry the inverted
   * incorrect-for-drops tag, so the forward mapping lives here.
   */
  private static TagKey<Block> tierTag(Tiers tier) {
    return switch (tier) {
  ...
  /** See {@link #tierTag(Tiers)}; forge's harvest levels for the vanilla tiers */
  ...
  private void tagPlanks(TagKey<Block> tag, boolean remove, WoodBlockObject... blocks) {
    for (WoodBlockObject block : blocks) {
      ...
      if (remove) {
        removeFromTag(tag, block.get());
        removeFromTag(tag, update);
      } else {
