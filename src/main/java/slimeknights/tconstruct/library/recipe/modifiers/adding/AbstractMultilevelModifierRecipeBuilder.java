  ...
  public T saveSalvage(RecipeOutput consumer, ResourceLocation id) {
    ...
    for (LevelEntry levelEntry : levels) {
      if (levelEntry.slots() != null) {
        ResourceLocation levelId = id.withSuffix("_level_" + levelEntry.level().min());
        consumer.accept(levelId, new ModifierSalvage(levelId, tools, maxToolSize, result, levelEntry.level(), levelEntry.slots()), null);
      }
    }
    return self();
  }
