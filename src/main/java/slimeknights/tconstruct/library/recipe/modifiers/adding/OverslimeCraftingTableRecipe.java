...
public class OverslimeCraftingTableRecipe extends CustomRecipe {
  ...
  /** Recipe id, no longer stored by vanilla recipes; kept for log messages and the loadable */
  ...
  public boolean matches(CraftingInput inv, Level level) {
    ...
    return tool.getModifierLevel(ContentLookups.OVERSLIME) > 0 || OverslimeModule.INSTANCE.getAmount(tool) < OverslimeModule.getCapacity(tool);
  }
  ...
  public RecipeSerializer<?> getSerializer() {
    return ContentLookups.recipeSerializer("crafting_overslime_modifier");
  }
}
