...
public class SlimeArmorLayer<T extends Slime, M extends HierarchicalModel<T>, A extends HumanoidModel<T>> extends RenderLayer<T,M> {
  /** Vanilla's fallback leather colour, for a dyeable piece that has no dye component */
  private static final int DEFAULT_LEATHER_COLOR = -6265536;
  /** Buffer the Tinkers model is handed; unused, as each of its layers picks its own */
  private static final ResourceLocation DEFAULT_ARMOR = ResourceLocation.withDefaultNamespace("textures/models/armor/leather_layer_1.png");
  ...
  public void render(PoseStack matrices, MultiBufferSource buffer, int packedLight, T entity, float pLimbSwing, float swing, float partialTicks, float age, float headYaw, float headPitch) {
    ...
    if (!helmet.isEmpty()) {
      ...
      if (item instanceof ArmorItem armor && armor.getType() == ArmorItem.Type.HELMET) {
        ...
        if (item instanceof ArmorModelItem modelItem) {
          // Tinkers armor names its own layers; Forge reached them through getArmorModel, which has
          // no Fabric counterpart outside an ArmorRenderer, and this is not one
          Model model = MultilayerArmorModel.INSTANCE.setup(entity, helmet, EquipmentSlot.HEAD, armorModel, ArmorModelManager.INSTANCE.getModel(modelItem.getArmorModelName()));
          MultiBufferSource previous = AbstractArmorModel.buffer;
          AbstractArmorModel.buffer = buffer;
          try {
            model.renderToBuffer(matrices, buffer.getBuffer(RenderType.armorCutoutNoCull(DEFAULT_ARMOR)), packedLight, OverlayTexture.NO_OVERLAY, -1);
          } finally {
            AbstractArmorModel.buffer = previous;
          }
        } else {
          // vanilla armor: one pass per material layer, tinting the dyeable ones. 1.21 replaced the
          // fixed "_layer_1" plus "_overlay" pair with this list, and the dye moved to a component.
          int dyed = helmet.is(ItemTags.DYEABLE) ? FastColor.ARGB32.opaque(DyedItemColor.getOrDefault(helmet, DEFAULT_LEATHER_COLOR)) : -1;
          for (ArmorMaterial.Layer layer : armor.getMaterial().value().layers()) {
            renderModel(matrices, buffer, packedLight, enchanted, armorModel, layer.dyeable() ? dyed : -1, layer.texture(false));
          }
        }
      } else {
        ...
        if (item instanceof BlockItem block && block.getBlock() instanceof AbstractSkullBlock skullBlock) {
          ...
          // 1.21 moved the skull owner from a SkullOwner tag onto the profile component
