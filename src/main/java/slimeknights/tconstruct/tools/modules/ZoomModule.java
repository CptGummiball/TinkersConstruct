...
public enum ZoomModule implements ModifierModule, GeneralInteractionModifierHook, KeybindInteractModifierHook, UsingToolModifierHook, EquipmentChangeModifierHook {
  ...
  private static void setZoom(ModifierEntry modifier, LivingEntity living, float amount) {
    TinkerDataCapability.run(living, data -> data.computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).set(modifier.getId(), amount));
  }

  /** Stops zooming */
  private static void stopZoom(ModifierEntry modifier, java.util.Optional<TinkerDataCapability.Holder> tinkerData) {
  ...
  private static void stopZoom(ModifierEntry modifier, LivingEntity entity) {
    TinkerDataCapability.run(entity, data -> data.computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).remove(modifier.getId()));
  }
