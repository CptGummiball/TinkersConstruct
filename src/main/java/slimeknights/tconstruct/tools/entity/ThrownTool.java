...
public class ThrownTool extends ThrownTrident implements ToolProjectile {
  ...
  public ThrownTool(Level level, LivingEntity shooter, ItemStack stack, float charge, float multiplier, float waterInertia) {
    ...
    this.pickupItemStack = stack.copyWithCount(1);
    this.charge = charge;
  ...
  private void updateFromStack() {
    this.entityData.set(STACK, pickupItemStack);
    this.entityData.set(ID_LOYALTY, (byte) ModifierUtil.getVolatileInt(pickupItemStack, LOYALTY));
    this.entityData.set(ID_FOIL, ModifierUtil.checkVolatileFlag(pickupItemStack, ModifiableItem.SHINY));
    this.noDespawn = ModifierUtil.checkVolatileFlag(pickupItemStack, IndestructibleItemEntity.INDESTRUCTIBLE_ENTITY);
    if (!level().isClientSide) {
      this.magnet = ModifierUtil.getVolatileInt(pickupItemStack, MAGNET);
    }
  }
  ...
  public void onRelease(LivingEntity entity, ModDataNBT arrowData) {
    ...
    for (ModifierEntry entry : tool.getModifierList()) {
      entry.getHook(ModifierHooks.PROJECTILE_THROWN).onProjectileShoot(tool, entry, entity, pickupItemStack, this, null, arrowData, true);
    }
    this.tasks = ScheduledProjectileTaskModifierHook.createSchedule(tool, pickupItemStack, this, null, arrowData);
  }
  ...
  /** If true, this tool summons lightning when hitting a lightning rod. 1.21 dropped the vanilla trident method (channeling moved to enchantment effects), so this is our own hook now. */
  public boolean isChanneling() {
    return !pickupItemStack.isEmpty() && getTool().getModifiers().getLevel(ModifierIds.channeling) > 0;
  }
  ...
  public Component getDisplayName() {
    ...
    IToolStackView tool = getTool();
    return ToolNameHook.getName(tool.getDefinition(), pickupItemStack, tool);
  }
  ...
  public void tickDespawn() {
    ...
    if (pickup != Pickup.ALLOWED || pickupItemStack.isEmpty()) {
      life += 1;
      if (life >= 1200) {
        this.discard();
      }
  ...
  protected void onBelowWorld() {
    ...
    if (pickup == Pickup.ALLOWED && this.entityData.get(ID_LOYALTY) != 0) {
      ...
      if (!pickupItemStack.isEmpty()) {
        ToolDamageUtil.damage(getTool(), 1, getOwner() instanceof LivingEntity l ? l : null, pickupItemStack);
      }
    } else {
      super.onBelowWorld();
    }
  ...
  private IToolStackView getTool() {
    if (tool == null) {
      tool = ToolStack.from(pickupItemStack);
    }
    return tool;
  }
  ...
  public void tick() {
    ...
    if (!dealtDamage && inGroundTime > 4) {
      // we don't damage the tool on throw, so instead damage it when it hits a block or an entity
      if (!pickupItemStack.isEmpty() && !level().isClientSide) {
        ToolDamageUtil.damage(getTool(), 1, getOwner() instanceof LivingEntity l ? l : null, pickupItemStack);
        ...
        this.entityData.set(STACK, pickupItemStack, true);
      }
      dealtDamage = true;
    }
    super.tick();
    ...
    if (!tasks.isEmpty() && !pickupItemStack.isEmpty()) {
      ScheduledProjectileTaskModifierHook.checkSchedule(getTool(), pickupItemStack, this, null, tasks);
    }
  }
  ...
  protected void onHitEntity(EntityHitResult pResult) {
    ...
    if (!pickupItemStack.isEmpty() && this.getOwner() instanceof LivingEntity owner) {
      ...
      if (ToolAttackUtil.canPerformAttack(tool) && ToolAttackUtil.isAttackable(owner, target)) {
        ...
        if (EntityInteractionModifierHook.meleeDisabled(tool)) {
          ...
          if (notSelf) {
            owner.setItemInHand(InteractionHand.OFF_HAND, pickupItemStack);
          }
  ...
  protected void onHitBlock(BlockHitResult result) {
    ...
    if (!hitBlock) {
      ...
      if (!dealtDamage && !pickupItemStack.isEmpty() && pickupItemStack.is(TinkerTags.Items.HARVEST) && this.getOwner() instanceof ServerPlayer owner) {
        ...
        if (!tool.isBroken()) {
          ...
          if (hardness != -1 && IsEffectiveToolHook.isEffective(tool, state)) {
            ...
            if (miningSpeed > 1.5 * hardness) {
              ...
              owner.setItemInHand(InteractionHand.MAIN_HAND, pickupItemStack);
              int harvested = ToolHarvestLogic.runBlockBreak(pickupItemStack, tool, state, pos, result.getDirection(), owner, this);
  ...
  public void readAdditionalSaveData(CompoundTag tag) {
    ...
    // update the tool to sync to client, if its set ("item" is AbstractArrow's pickup stack key in 1.21)
