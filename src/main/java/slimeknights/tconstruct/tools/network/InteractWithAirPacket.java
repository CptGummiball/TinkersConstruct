    ...
    if (player != null && !player.isSpectator()) {
      if (this == LEFT_CLICK) {
        ...
        if (held.is(TinkerTags.Items.INTERACTABLE_LEFT)) {
          InteractionResult result = InteractionResult.PASS; // PORT (event layer): InteractionHandler.onLeftClickInteraction
        ...
        if (chestplate.is(TinkerTags.Items.INTERACTABLE_ARMOR) && player.getItemInHand(hand).isEmpty()) {
          InteractionResult result = InteractionResult.PASS; // PORT (event layer): InteractionHandler.onChestplateUse
          if (result.shouldSwing()) {
            player.swing(hand, true);
          }
        }
